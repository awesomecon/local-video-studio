...
- Ask the user for approval before spawning subagents. Do not delegate work unless the user has
  explicitly approved subagent use for the task.
...
- During parallel development, the main checkout is the integration workspace and owns
  `pyproject.toml`, lock files, this file, `.gitignore`, README, shared contracts, dependency
...
The following owned-path assignments apply only when parallel workers are being used. A single
agent may make authorized changes across these paths without creating separate workers.

Owned paths for parallel workers:
...
- Run focused tests while developing, then the full suite with `python -m pytest` before requesting
  review of Python or test/CI changes. Validate frontend changes with
  `python3 frontend/tests/static_checks.py`. Documentation-only changes do not require a test run.
...
4. Set `LVS_CHROME` when Chromium is not discoverable automatically. The script uses a throw-away
   profile and requires `websocket-client`, a regular application dependency. Never reuse the user's
   real Chrome profile, and never point the browser at non-localhost origins.
