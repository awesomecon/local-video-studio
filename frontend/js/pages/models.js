/**
 * Models screen: what this machine can run, what is loaded, and which
 * model is selected.
 *
 *  - Everything is data-driven from GET /api/system/status and
 *    GET /api/models — no model IDs, GPU names, or versions are hardcoded.
 *  - Script model selection (local LLM router), registered generator
 *    backends (with heavyweight flag, VRAM requirement, and runtime
 *    state), the Studio-owned model memory controls, MiniMax H3
 *    cold-load readiness, and the live GPU snapshot.
 *  - Environment and machine diagnostics (core capabilities,
 *    compatibility, project recovery, runtime environment, ports) live on
...
export function renderModels(_route) {
  const screen = el("div", { class: "screen" },
    el("div", { class: "screen-head" }, el("h1", {}, "Models")),
  );
  if (state.connection !== "online") {
    screen.append(errorPanel(
      { kind: "offline", message: "Backend offline — model status is unavailable." },
...
function modelsPanel() {
  ...
  const panel = el("div", { class: "panel" },
    el("div", { class: "row" },
      el("span", { class: "panel-title" }, "Model status"),
